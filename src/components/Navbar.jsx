...
<a aria-label="GitHub" href={profile.socials.github} className="text-white/70 hover:text-white"><Github size={18}/></a>
<a aria-label="LinkedIn" href={profile.socials.linkedin} className="text-white/70 hover:text-white"><Linkedin size={18}/></a>
<a aria-label="Website" href={profile.socials.website} className="text-white/70 hover:text-white"><Globe size={18}/></a>
</nav>
<button className="md:hidden link-btn" onClick={() => setOpen(v => !v)} aria-label="Toggle menu">
...
{nav.map(n => (
<a key={n.href} href={n.href} onClick={() => setOpen(false)} className="text-white/90">{n.label}</a>
))}
</div>
</div>
)}
